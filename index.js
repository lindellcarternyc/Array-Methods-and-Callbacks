...
const [finalMatch] = fifaData.filter(fifaMatch => {
    return fifaMatch.Year === 2014 && fifaMatch.Stage === 'Final'
})
...
function getYears(data, getFinalsCb) {
    return getFinalsCb(data).map(match => match.Year)
}
...
function getWinnersByYear(array, getYearsCb, getWinnersCb) {
    ...
    return years.map((year, idx) => {
        return `In ${year}, ${winners[idx]} won the world cup!`
    })
}
...
function getAverageGoals(data) {
   const matchGoals = data.reduce((total, match) => {
        return total + match['Home Team Goals'] + match['Away Team Goals']
    }, 0)

    return (matchGoals / data.length).toFixed(2)
